    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16 border-l-2 border-lime-600">
        <main class="container mx-auto bg-transparent mt-8">
            <table class="w-full text-left border-collapse border border-gray-300">
                <thead>
                    <tr>
                        <th class="p-4 border-b border-gray-300 font-bold text-blue-700">Title</th>
                        <th class="p-4 border-b border-gray-300 font-bold text-blue-700">URL</th>
                        <th class="p-4 border-b border-gray-300 font-bold text-blue-700">Description</th>
                ...
                <tbody>
                    @foreach($video as $item)
                        <tr>
                            ...
                            <td class="p-4 border-b border-gray-200">{{ $item->description }}</td>

                            <td class="p-4 border-b border-gray-200">
                                <a href="{{ route('videos.edit', $item->id) }}" class="bg-green-400 p-2 rounded-lg ring-1">Edit</a>
                                
                                <form action="{{ route('videos.destroy', $item->id) }}" method="post" class="inline" onsubmit="return confirmDelete();">
                                    @csrf
                                    @method('DELETE')
