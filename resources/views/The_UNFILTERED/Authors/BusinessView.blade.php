            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                @foreach($business as $item)
                    <article class="mb-4 rounded-lg ">
                        <a href="{{ route('business.show', $item->slug) }}" class="block p-4 mt-4"> <header>
                        ...
                        @endif
                                                ...
                                                <p class="text-lg font-bold text-blue-900 mt-2"><a class="ring-1 rounded-lg bg-green-400 p-2 " href="{{route('business.edit',$item->id)}}">Edit</a></p>
                                                
                                                <form action="{{ route('business.destroy', $item->id) }}" method="get" onsubmit="return confirmDelete();">
                                                    @csrf
                                                    @method('DELETE')
